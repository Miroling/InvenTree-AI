...
Required deployment targets are Docker Compose and Helm on k3s. Public inventory access must require authenticated and authorized users from ChatGPT, Codex, and Claude. Login to an AI client is not authorization to this service. The owner selected https://github.com/h4ckf0r0day/obscura as the browser engine for component research. PinchTab is excluded, including fallback paths. Earlier research recommendations favoring PinchTab are historical and superseded by this explicit selection. Optimize Obscura extraction and report its limitations without reopening the provider choice.
